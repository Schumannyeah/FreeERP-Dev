$(document).ready(function() {    
    ...
    $('#tableMesWorkOrdersListList tfoot th').each(function () {
        var title = $(this).text();
        $(this).html('<input type="text" placeholder="' + title + '" />');
    });
    
    // $(window).resize(setTimeout(respondCanvas, 500));
    // showMesWorkOrderList();
    ...
    // $("#showWoGantt").click(hideFaufGantt());

    hidePickDiv();
    
} );

function showMesWorkOrderList(){
    var myMWOL = $('#tableMesWorkOrdersListList').DataTable({
            "destroy": true,
            "processing": true,
            "ordering": true,
            "info": true,
            ...
            "ajax": {
                "url": "/manufacturing/control/ShowMesWorkOrders",
                "dataSrc": function(d){
                    return d.listWorkOrders;
                },
                "type": "POST"
            },
            // hidden columns is not working
            // "columnDefs": [
            //     {
            //         target: 24,
            //         visible: false,
            ...
            "columns": [
                ...
                { data: "PICKED_STATUS" },
                { data: "ATP_STATUS" },
                { data: "WAREHOUSE_OPERATOR" },
                { data: "NAME_PLATE" },

                {
                	"orderable": true,
		            "data": "WO_CREATED_DATE",
		            "render": function (date) {
		            	return moment(date).format("YYYY-MM-DD");
		            }
                },
            ...
            "createdRow": function( row, data, dataIndex ) {
                ...
                var tdProductEndBy = $(row).children('td:eq(24)');
	           	...
	           	var tdNamePlate = $(row).children('td:eq(32)');
                ...
                if (woOrderStatus == "PRUN_COMPLETED") {
                    ...
                    // Color name plate
                    if (namePlate == "YES") {
                        $(tdNamePlate).addClass('rowAttention');
                    } 
                }
        ...
        $('div.toolbar').html('<b>Only Showing Production Orders Scheduled in MES.</b>');

        $('#tableMesWorkOrdersListList tbody').on('mouseenter', 'td', function () {
            var colIdx = myMWOL.cell(this).index().column;
     
            $(myMWOL.cells().nodes()).removeClass('highlight');
            $(myMWOL.column(colIdx).nodes()).addClass('highlight');
        });

        // Add event listener for opening and closing details
        $('#tableMesWorkOrdersListList tbody').on('click', 'td.details-control', function () {
            var tr = $(this).closest('tr');
            var tdi = tr.find("i.fa");
            var row = myMWOL.row(tr);
            ...
            if (row.child.isShown()) {
                ...
                var myNamePlate = row.data().NAME_PLATE;
                // console.log("----");
                // console.log(prodCommittedDateSelected);
                // console.log(isDate(prodCommittedDateSelected));
                // console.log("*****");
                // console.log(moment((moment(prodCommittedDateSelected).toDate())).format("MM/DD/YYYY"));
                // console.log(isDate(moment().toDate(prodCommittedDateSelected)));
        ...
        myMWOL.on("user-select", function (e, dt, type, cell, originalEvent) {
        ...
        $('#tableMesWorkOrdersListList tbody').on( 'click', 'tr', function () {
            $(this).toggleClass('selected');
        } );

        // // -------------- temp removing for not very practical
        // // for highchart integration
        // // Create the chart with initial data
        // var container = $('<div/>').insertAfter(myMWOL.table().container());
        ...
        //     },
        //     title: {
        //         text: 'Production Order Count Per Pool',
        //     },
        //     series: [
        //         {
        //             data: chartData(myMWOL),
        //         },
        //     ],
        // });
    
        // // On each draw, update the data in the chart
        // myMWOL.on('draw', function () {
        //     chart.series[0].setData(chartData(myMWOL));
        // });
...
function format(d){
    ...
    return '<table class="popout" cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">' +
        '<tr>' +
            '<td><input type="hidden" id="myRowID" value="'+ d.WO +'"></td>' +
            '<td colspan="9"><input type="hidden" id="myFaufNr' + d.WO + '" value="'+ d.WO +'"></td>'+
            '<td> <button id="btnEditWoDateAndNote" type="button" class="rowOk" onclick="editFaufDateAndNote()">Production Or ME</button> </td>' +
            ...
            '<td> <button id="btnConfDate" type="button" class="rowNotADate" onclick="showDateConf()">Confirm Date By Other Function</button> </td>' +
            '<td> <button id="btnShowPickStatus" type="button" class="rowLightSkyBlue" onclick="showPickStatus()">Show Picking Status</button> </td>' +
            '<td colspan="9"><input type="hidden" id="myNamePlate"></td>'+
            '<td> <button id="btnLogToNamePlate" type="button" class="rowTeal" onclick="logToNamePlate()">Name Plate Log</button> </td>' +
        '</tr>' +	        
    '</table>';  
}
...
function hidePoolGantt(){
    $("#showPoolGanttModal").modal("hide");
...
function logToNamePlate(){
    var myNamePlate = $("#myNamePlate").val();
    var myProdId = $("#myRowID").val();
    if (myNamePlate == "YES"){
        window.open("/quality/control/GenNamePlateLabel?prodId=" + myProdId + "&myNamePlate=" + myNamePlate);
    }
}
...
// temporarily not used and be replaced by function branchToFormDateAndNote
...
// var WebAppUtil = Packages.org.apache.ofbiz.webapp.WebAppUtil;

function branchToProductionConf() {
    var prodId = $("#faufId").val();
	// var committedDate = $("#faufCommittedDate").val();
	// var faufMesNotes = $("#faufMesNotes").val();
    // window.location.href = "/manufacturing/control/EditFaufNotes";
    $("#faufCommitDateAndNoteModal").modal("hide");
    window.open("/manufacturing/control/EditFaufNotes?prodId=" + prodId);
}

function branchToMeConf() {
    var prodId = $("#faufId").val();
    var mesUser = getCookie("mesUser");
    console.log(mesUser);
	// var committedDate = $("#faufCommittedDate").val();
	// var faufMesNotes = $("#faufMesNotes").val();
    // window.location.href = "/manufacturing/control/EditFaufNotes";
    $("#faufCommitDateAndNoteModal").modal("hide");
    window.open("/manufacturing/control/EditMBomEnd?prodId=" + prodId + "&meEndBy=" + mesUser);
}
