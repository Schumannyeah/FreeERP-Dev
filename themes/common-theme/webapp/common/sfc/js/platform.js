...
function nearBestPosition(whereId, theObjId, centerOnEl) {
  ...
  if (el) {
    ...
    // modified by Schumann 20211102 for displaying right
    // position of calendar selector for gantt End Date
    // var trueX = hasContainment ? el.position().left : el.offset().left;
    // var trueY = hasContainment ? el.position().top : el.offset().top;
    var trueX = hasContainment ? el.offset().left : el.position().left;
    var trueY = hasContainment ? el.offset().top : el.position().top;
...
jQuery.fn.createErrorAlert = function(errorCode, message) {
  this.each(function() {
    ...
    if ($("#" + el.attr("id") + "error").size() <= 0) {
      ...
      err=$(err);
      err.attr("title",errMess).attr("src","/common/sfc/res/alert.gif");
      el.after(err);
    }
  });
  return this;
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  if(window.name!=iframeId)
    ...
    bg.css({backgroundImage:"url('/common/sfc/res/black_70.png')"});
  ...
  bdiv.append("<img src=\"/common/sfc/res/closeBig.png\" style='cursor:pointer;position:absolute;right:-40px;top:30px;'>");
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  bg.css({position:'fixed',top:"0px",paddingTop:"50px", left:0,width:'100%',height:'100%',  backgroundImage:"url('/common/sfc/res/black_70.png')"});
  ...
  var img=$("<img src='/common/sfc/res/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
  bdiv.append(img);
  img.click( function(){
    bg.trigger("close");
  });
